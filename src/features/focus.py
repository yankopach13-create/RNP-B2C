...
def build_focus_display_df(
    ...
    category_order_focus = list(
        dict.fromkeys(
            focus_df["Категория фокуса"].dropna().astype(str).str.strip().tolist()
        )
    )
    names_focus = focus_df["Фокусные позиции ур. 3"].dropna().tolist()
    names_focus_set = set(str(x).strip() for x in names_focus)
    cat_map_focus = dict(
        zip(focus_df["Фокусные позиции ур. 3"], focus_df["Категория фокуса"])
    )
    ...
    df_focus_sales = df_sales[focus_mask].copy()
    ...
    df_focus_sales.loc[mask_missing, "Группа_отчет"] = (
        df_focus_sales.loc[mask_missing, "Группа"].fillna("").astype(str).str.strip()
    )
    ...
    ordered_categories = (
        [cat for cat in category_order_focus if cat in pivot.columns]
        + [cat for cat in pivot.columns if cat not in category_order_focus]
    )
    pivot = pivot.reindex(columns=ordered_categories, fill_value=0)

    overall = pivot.sum(axis=0)
